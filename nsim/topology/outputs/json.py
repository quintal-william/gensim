...
class JsonTopologyOutput(JsonOutput[Node]):
    def __make_edge_data(self, edge: Edge) -> Json:
        data = self._make_item(edge.__class__.__name__, edge.get_id())
    ...
    def __make_leaf_data(self, leaf: Leaf) -> Json:
        data = self._make_item(leaf.get_type().value, leaf.get_id())
    ...
    def __make_topology_data(self, topology: Topology) -> Json:
        data = self._make_item(topology.__class__.__name__, topology.get_id())
    ...
    def __make_node_data(self, node: Node) -> Json:
        ...
        return self._make_item(node.__class__.__name__, node.get_id())
